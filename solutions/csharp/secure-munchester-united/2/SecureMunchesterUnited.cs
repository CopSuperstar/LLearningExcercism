...
{
    if (support is Chairman || support is Manager)
    {
        return "Too Important for a Security Pass";
    }
    if (support is Security)
        {
            if (support is SecurityJunior ||support is PoliceLiaison ||support is SecurityIntern)
            {
                if (support is SecurityJunior) return "Security Junior";
                 if (support is SecurityIntern) return "Security Intern";
                 if (support is PoliceLiaison) return "Police Liaison Officer";
            }
            if (support.GetType() is Security)
            {
                  return $"{support.GetType().Name} Priority Personnel";
            }
            else 
            {
    ...
    return $"The {support.GetType().Name}"; 
}
